...
const processHourlyExpiringSubscriptions = async () => {
    try {
        ...
        for (const subscription of subscriptions) {
            ...
            Logger.info(`🔍 Subscription ${subscription._id}: ${minutesRemaining} minutes remaining until expiry.`);

            // Check if already sent
            if (subscription?.vpsPlanReminders?.renewal?.firstReminderSent) {
                Logger.info(`⏭️ Skipping subscription ${subscription._id}, already sent reminder.`);
                continue;
            }

            if (subscriptionEnd.isBefore(currentDate.clone().add(reminderTime, 'minutes'))) {
                const user = subscription.userId;
                Logger.info(`📧 Sending hourly reminder for subscription ${subscription._id} to ${user.email}`);
                ...
                Logger.info(`✅ Hourly reminder sent to ${user.email} for subscription ${subscription._id}`);
            }
        }
    } catch (error) {
        Logger.error('❌ Error checking hourly subscriptions:', error);
    }
};
...
const processOtherExpiringSubscriptions = async () => {
    try {
        ...
        for (const subscription of subscriptions) {
            ...
            if (reminderDays === 0) {
                Logger.info(`⏭️ Skipping subscription ${subscription._id}, ${dayDifference} not within reminder range.`);
                continue;
            }
